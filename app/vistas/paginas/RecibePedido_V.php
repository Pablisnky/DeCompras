<section class="section_2">
    <h1 class="h1_1 h1_12">Compra realizada</h1>
    <div class='contenedor_39'>
        <p class="p_3">Se envio a tu correo la orden de compra. </p> 
        <a class="a_6 a_11" href="#">Ver orden de compra</a>
        <P class="p_1">En breve se te notificará los datos del despachador y hora aproximada de entrega </P>
        <div class="contenedor_33">
            <p class="p_1">Gracias por confiar en nuestro servicio</p>
            <a class="boton" href="<?php echo RUTA_URL . '/Inicio_C';?>">Inicio</a>
        </div>
    </div>
</section>