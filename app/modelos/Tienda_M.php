    ...
    class Tienda_M extends Conexion_BD{
        ...
        public function consultarTiendas($Categoria, $Ciudad){
            $stmt = $this->dbh->prepare(
                ...
                WHERE categorias.categoria = :CATEGORIA AND parroquia_Tien = :PARROQUIA  AND publicar_Tien = :PUBLICAR ORDER BY nombre_Tien"
            );      
        ...
        public function consultarProductosDestacados($IDs_Tiendas){
            $stmt = $this->dbh->prepare(
                "SELECT nombre_img, precioBolivar
                 ...
                 WHERE secciones.ID_Tienda IN ($IDs_Tiendas) AND productos.destacar = 1 AND imagenes.fotoPrincipal = 1  ORDER BY producto"
            );      
